"""Stage M acceptance tests for rbsummit's sensor migration (.claude/parametrized-robots.md
...
GOLDEN = Path(__file__).parent / "golden" / "rbsummit_premigration.urdf"
...
class TestEffectiveSensorsParity:
    ...
    def test_effective_sensors_matches_model_params(self) -> None:
        view = RobotIdentifier("rbsummit").resolve_sync()
        assert view.assembly is not None, "rbsummit must have migrated (assembly.yaml present)"
...
class TestUrdfStructuralParity:
    ...
    def test_wrapper_render_matches_golden(self, tmp_path: Path) -> None:
        ...
        wrapper_path = tmp_path / "rbsummit_wrapper.urdf.xacro"
        ...
        rendered_path = tmp_path / "rbsummit_wrapper.urdf"
