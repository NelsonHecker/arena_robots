"""Stage M acceptance tests for rbtheron's sensor migration (.claude/parametrized-robots.md
sec2.8, Stage M step 5): the pilot robot's default config must render identically
...
GOLDEN = Path(__file__).parent / "golden" / "rbtheron_premigration.urdf"


class TestEffectiveSensorsParity:
    """The pilot's acceptance test (sec2.8): default-request resolution reproduces
    ...
    def test_effective_sensors_matches_model_params(self) -> None:
        ...
        assert view.assembly is not None, "rbtheron must have migrated (assembly.yaml present)"
...
class TestUrdfStructuralParity:
    ...
    def test_wrapper_render_matches_golden(self, tmp_path: Path) -> None:
        ...
        wrapper_path = tmp_path / "rbtheron_wrapper.urdf.xacro"
        ...
        rendered_path = tmp_path / "rbtheron_wrapper.urdf"
