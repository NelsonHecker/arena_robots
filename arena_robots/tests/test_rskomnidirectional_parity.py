"""Stage M acceptance tests for rskomnidirectional's sensor migration
...
GOLDEN = Path(__file__).parent / "golden" / "rskomnidirectional_premigration.urdf"
...
class TestEffectiveSensorsParity:
    ...
    def test_effective_sensors_matches_model_params(self) -> None:
        view = RobotIdentifier("rskomnidirectional").resolve_sync()
        assert view.assembly is not None, "rskomnidirectional must have migrated (assembly.yaml present)"
...
class TestUrdfStructuralParity:
    ...
    def test_wrapper_render_matches_golden(self, tmp_path: Path) -> None:
        ...
        wrapper_path = tmp_path / "rskomnidirectional_wrapper.urdf.xacro"
        ...
        rendered_path = tmp_path / "rskomnidirectional_wrapper.urdf"
