"""Stage M acceptance tests for husky's sensor migration
...
GOLDEN = Path(__file__).parent / "golden" / "husky_premigration.urdf"
...
class TestEffectiveSensorsParity:
    ...
    def test_effective_sensors_matches_model_params(self) -> None:
        view = RobotIdentifier("husky").resolve_sync()
        assert view.assembly is not None, "husky must have migrated (assembly.yaml present)"
...
class TestUrdfStructuralParity:
    ...
    def test_wrapper_render_matches_golden(self, tmp_path: Path) -> None:
        ...
        wrapper_path = tmp_path / "husky_wrapper.urdf.xacro"
        ...
        rendered_path = tmp_path / "husky_wrapper.urdf"
